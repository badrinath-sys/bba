...
function s3upload($source, $dest, $remarks = "", $type = "html") {
    ...
    // $logfl = "/s3upload_logs/log-" . date("Ymd") . ".txt";
    //$logfl = "/var/www/html/EtvBharatPrimeApps/mahilabharat/s3upload_logs/log-" . date("Ymd") . ".txt";
	$logfl = "/var/www/html/EtvBharatPrimeApps/balbharat/s3upload_logs/log-" . date("Ymd") . ".txt";
    $fp = fopen($logfl, "a");
    ...
    try {
        $s3Client = new S3Client([
            ...
            'version' => 'latest',
            'credentials' => [
                'key' => 'AKIASAJGPVRPOVP6UHWD',
                'secret' => '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'
            ]
        ]);
