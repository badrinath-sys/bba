...
Class Facebook {
    ...
    public function __construct() {
        ...
        $this->config['facebook_app_id'] = '616874248922483';
        $this->config['facebook_app_secret'] = 'a27156312dabbd85ad5e4e38f2d830d9';
        $this->config['facebook_login_type'] = 'web';
        $this->config['facebook_login_redirect_url'] = $this->config->base_url() . 'fb_login';
        $this->config['facebook_logout_redirect_url'] = $this->config->base_url() . 'fb_logout';
        $this->config['facebook_permissions'] = array('email');
        $this->config['facebook_graph_version'] = 'v3.10';
        $this->config['facebook_auth_on_load'] = TRUE;
        ...
        switch ($this->config['facebook_login_type']) {
            case 'js':
        ...
        if ($this->config['facebook_auth_on_load'] === TRUE) {
    ...
    public function login_url() {
        ...
        if ($this->config['facebook_login_type'] != 'web') {
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
                        base_url() . $this->config['facebook_login_redirect_url'], $this->config['facebook_permissions']
        );
    }
    ...
    public function logout_url() {
        ...
        return $this->helper->getLogoutUrl(
                        $this->get_access_token(), base_url() . $this->config['facebook_logout_redirect_url']
        );
    }
    ...
    private function get_access_token() {
        return (isset($_SESSION['fb_access_token']) ? $_SESSION['fb_access_token'] : '');
    }
    ...
    private function get_expire_time() {
        return (isset($_SESSION['fb_expire']) ? $_SESSION['fb_expire'] : "");
    }
    ...
    private function set_expire_time(DateTime $time = null) {
        if ($time) {
            $_SESSION['fb_expire'] = $time->getTimestamp();
        }
    }
...
//        return array('error' => $code, 'message' => $message);
        echo '<br/>' . array('error' => $code, 'message' => $message);
    }
